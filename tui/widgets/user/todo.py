...
class TodoList(Vertical):
    def __init__(self):
        ...
        try:
            ...
            if not self.todo_list_api:
                print("Warning: Todo List API not available")
                self.logger.log("Todo List API not available", "WARNING", "init()", "TodoList")
        except Exception as e:
            self.logger.log(f"Error initializing Todo API: {e}", "ERROR", "init()", "TodoList")
    ...
    def switch_content(self, content_id: str):
        ...
        try:
            content_switcher = self.query_one("#content-switcher-todolist", ContentSwitcher)
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        task_viewer = self.query_one(self.TaskViewer)
        ...
        if event.button.id == "confirm-add-task-btn":
            try:
                ...
                self.switch_content("tasks-main-view")
